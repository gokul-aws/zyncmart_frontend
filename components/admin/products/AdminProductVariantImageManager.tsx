...
import { useUploadAdminVariantImages, useDeleteAdminVariantImage } from '@/hooks/useAdminProducts';
...
interface AdminProductVariantImageManagerProps {
  productId: string;
  productSlug: string;
  variantId: string;
  color: string;
...
// Same upload/preview/delete mechanics as AdminProductImageManager, scoped to
// one color variant's own image gallery via the variant-specific endpoints.
export default function AdminProductVariantImageManager({
  productId,
  productSlug,
  variantId,
  color,
  colorCode,
  images,
}: AdminProductVariantImageManagerProps) {
  ...
  const uploadMutation = useUploadAdminVariantImages(productSlug);
  const deleteMutation = useDeleteAdminVariantImage(productSlug);
    ...
    >
      <div className="flex items-center justify-between gap-3 border-b border-slate-100 px-5 py-4 dark:border-slate-800">
        <div className="flex items-center gap-2">
          {colorCode && (
            <span
              className="h-5 w-5 flex-shrink-0 rounded-full border border-slate-200 dark:border-slate-700"
              style={{ backgroundColor: colorCode }}
            />
          )}
          <div>
            <h2 className="text-sm font-semibold text-slate-900 dark:text-white">{color}</h2>
                ...
                >
                  <div className="relative aspect-square w-full">
                    ...
                    <button
                      type="button"
                      title="Delete image"
                      onClick={() => deleteMutation.mutate({ productId, variantId, publicId: image.publicId })}
